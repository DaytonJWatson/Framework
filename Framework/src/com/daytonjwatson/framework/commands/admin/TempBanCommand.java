...
public class TempBanCommand extends BaseCommand {
    public TempBanCommand(FrameworkPlugin plugin, FrameworkAPI api, StorageManager storage, PlayerDataManager playerData, MessageHandler messages) {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        sender.sendMessage(messages.getMessage("tempban-success")
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        if (args.length == 0) {
            return Collections.singletonList(messages.getMessage("tempban-usage"));
        }
        if (args.length == 1) {
